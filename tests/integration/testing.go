...
func newDockerServiceInstance(res *dockertest.Resource, dockerHost, host string) *DockerServiceInstance {
	cleanup := func() {
		err := pool.Purge(res)
		if err != nil {
			log.Panic(fmt.Errorf("error purging container %s - %w", res.Container.Name, err))
		}
	}
...
func createPool() {
	...
	if err != nil {
		log.Panic(fmt.Errorf("error connecting to docker - %w", err))
	}
}
...
// // NewRedisService returns a redis service.
// func NewRedisService(withinDocker bool) *DockerService {
// 	dockerHostname := ""
// 	if !withinDocker {
...
// 		ContainerPort:  "6379",
// 		Env:            []string{},
// 		Cmd:            []string{"redis-server", "--requirepass", redis.RedisPass},
...
func (svc *DockerService) Start() (*DockerServiceInstance, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("healthcheck failed for %s: %w", svc.Image, err)
	}
